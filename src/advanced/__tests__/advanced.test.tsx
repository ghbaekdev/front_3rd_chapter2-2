...
import ProductForm from "../../refactoring/components/Admin/ProductForm";
import CouponForm from "../../refactoring/components/Admin/CouponForm";
import { useProductList } from "../../refactoring/hooks/useProductList";
import {
  getAppliedDiscount,
  getMaxDiscount,
  getRemainingStock,
...
describe("advanced > ", () => {
  ...
  test("CouponForm 테스트", () => {
    const handleAddCoupon = vi.fn();
    render(<CouponForm handleAddCoupon={handleAddCoupon} />);
    ...
    expect(handleAddCoupon).toHaveBeenCalledWith({
      name: "테스트 쿠폰",
      code: "TEST123",
      discountType: "percentage",
      discountValue: "15",
    ...
    // 3. 입력 필드 초기화
    expect(screen.getByPlaceholderText("쿠폰 이름")).toHaveValue("");
    expect(screen.getByPlaceholderText("쿠폰 코드")).toHaveValue("");
    expect(screen.getByPlaceholderText("할인 값")).toHaveValue(0);
  });

  test("ProductForm 테스트", () => {
    const handleAddNewProduct = vi.fn();
    render(<ProductForm handleAddNewProduct={handleAddNewProduct} />);
    ...
    expect(handleAddNewProduct).toHaveBeenCalledWith({
      name: "테스트 상품",
      price: "10000",
      stock: "50",
      discounts: [],
    ...
    expect(screen.getByLabelText("재고")).toHaveValue("0");
  });
});

describe("discountUtils 순수함수 테스트 > ", () => {
  ...
  test("getRemainingStock 함수 테스트", () => {
    ...
    const cart: CartItem[] = [{ product: product, quantity: 5 }];
    expect(getRemainingStock(product, cart)).toBe(15);
  });
  ...
  test("getAppliedDiscount 함수 테스트", () => {
    ...
    const cartItem: CartItem = { product: product, quantity: 7 };
    expect(getAppliedDiscount(cartItem)).toBe(0.05);
  });
});

describe("useProductList 훅 테스트 > ", () => {
  const mockProducts: Product[] = [
    {
      id: "p1",
      name: "상품1",
  ...
  test("상품 아코디언 토글 기능", () => {
    const { result } = renderHook(() =>
      useProductList(mockProducts, mockOnProductUpdate)
    );
  ...
  test("상품 수정 기능", () => {
    ...
    act(() => {
      result.current.handleProductUpdate("p1", { name: "수정된 상품1" });
    });
  ...
  test("할인 추가 및 제거 기능", () => {
    ...
    act(() => {
      result.current.setNewDiscount({ quantity: 5, rate: 0.05 });
    });
    ...
    act(() => {
      result.current.handleRemoveDiscount("p2", 0);
    });
